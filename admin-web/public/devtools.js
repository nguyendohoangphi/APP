...
const btnSeedStores = document.getElementById('btn-seed-stores');
if (btnSeedStores) {
    btnSeedStores.onclick = async () => {
        if (!confirm("Bạn có muốn tạo dữ liệu 3 cửa hàng mẫu không?")) return;

        const btnText = btnSeedStores.innerHTML;
        btnSeedStores.disabled = true;
        btnSeedStores.innerHTML = "Đang tạo...";

        try {
            const dummyStores = [
                {
                    name: "Coffee Shop - 02 Ông Ích Khiêm",
                    address: "02 Ông Ích Khiêm, P. Thanh Bình, Quận Hải Châu, Đà Nẵng",
                    lat: 16.0799,
                    lng: 108.2166,
                    image: "assets/images/banner/firstbanner.jpg",
                    province: "Đà Nẵng",
                    district: "Hải Châu",
                },
                {
                    name: "Coffee Shop - 19 Nguyễn Cơ Thạch",
                    address: "19 Nguyễn Cơ Thạch, Phường Mỹ Đình 2, Quận Nam Từ Liêm, Hà Nội",
                    lat: 21.0347,
                    lng: 105.7667,
                    image: "assets/images/banner/secondbanner.jpg",
                    province: "Hà Nội",
                    district: "Nam Từ Liêm",
                },
                {
                    name: "Coffee Shop - Lê Lợi HCM",
                    address: "45 Lê Lợi, Bến Nghé, Quận 1, Hồ Chí Minh",
                    lat: 10.7769,
                    lng: 106.7008,
                    image: "assets/images/banner/thirdbanner.jpg",
            ...
            dummyStores.forEach(store => {
                ...
                batch.set(newStoreRef, store);
            });

            await batch.commit();
            alert("Đã tạo thành công 3 cửa hàng mẫu!");
        } catch (error) {
            console.error(error);
            alert("Lỗi khi tạo dữ liệu: " + error.message);
        } finally {
            btnSeedStores.disabled = false;
            btnSeedStores.innerHTML = btnText;
        }
    };
}
